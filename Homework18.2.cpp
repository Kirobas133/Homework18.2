...
public:
    explicit OneShotDB(std::unique_ptr<VeryHeavyDatabase> real_object, size_t shots = 1) : real_db_(std::move(real_object)), shots_(shots) {
...
int main() {
    ...
    */
    std::unique_ptr<VeryHeavyDatabase> real_db = std::make_unique<VeryHeavyDatabase>();
